        ...
        <thead>
          <tr>
            ...
            <th style="width: 40%" @click="orderBy('number')">
              학번
              <i class="mdi mdi-arrow-down" :class="{ 'order-active': sortKey == 'number', 'order-rotate': sortBy }"></i>
            </th>
            <th style="width: 50%" @click="orderBy('owner.name')">
              이름
              <i class="mdi mdi-arrow-down" :class="{ 'order-active': sortKey == 'owner.name', 'order-rotate': sortBy }"></i>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(application, idx) in getOrderedApplicants" :key="application._id" :class="{ active: currentApplicant == idx }" @click="currentApplicant = idx">
            <td>{{ idx + 1 }}</td>
