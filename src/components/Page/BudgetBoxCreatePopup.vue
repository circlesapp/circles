<template>
  <div class="budget__createpopup">
    <form class="budget__createpopup__content" action="javascript:void(0);" @submit="create">
      <button class="budget__createpopup__content__clear" type="button" @click="$emit('isUpdated', false)">
      ...
      <div class="budget__createpopup__content__inputs">
        <div class="inputfield">
          <h4>품목</h4>
          <input ref="startInput" v-model="item" class="inputfield__input" type="text" placeholder="품목을 입력하세요" required />
        </div>
        <div class="inputfield">
          <h4>규격</h4>
        ...
        <div class="inputfield">
          <h4>단가</h4>
          <input v-model="price" class="inputfield__input" type="number" placeholder="단가를 입력하세요" required />
        </div>
        <div class="inputfield">
          <h4>수량</h4>
          <input v-model="quantity" class="inputfield__input" type="number" placeholder="수량을 입력하세요" required />
        </div>
        <div class="inputfield">
          <h4>배송료</h4>
          <input v-model="shopping" class="inputfield__input" type="number" placeholder="배송료를 입력하세요" required />
        </div>
        <div class="inputfield">
          <h4>URL</h4>
...
@Options({})
export default class BudgetBoxCreatePopup extends Vue {
  item: string = '';
  size: string = '';
  price: number = 0;
  quantity: number = 0;
