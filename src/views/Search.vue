<template>
  <div class="search">
    ...
    <transition-group name="clubs" class="search__list" tag="div">
      <router-link :to="`/${club.name}/page/timeline`" class="search__list__item" v-for="club in filterClub" :key="club._id">
  ...
  watch: {
    search() {
      ...
      this.searchTimer = setTimeout(() => {
        this.filterClub = this.clubs.filter((club: any) => club.name.toLowerCase().indexOf(this.search.toLowerCase()) != -1 || club.school.toLowerCase().indexOf(this.search.toLowerCase()) != -1);
      }, 300);
    },
  },
})
