<template>
  <div class="award__createpopup">
    <form class="award__createpopup__content" action="javascript:void(0);" @submit="create">
      <button class="award__createpopup__content__clear" type="button" @click="$emit('isUpdated', false)">
      ...
      <div class="award__createpopup__content__inputs">
        <div class="inputfield">
          <h4>대회명</h4>
          <input ref="startInput" v-model="title" class="inputfield__input" type="text" placeholder="대회명을 입력하세요" required />
        </div>
        <div class="inputfield">
          <h4>부문</h4>
          <input v-model="subtitle" class="inputfield__input" type="text" placeholder="부문을 입력하세요" required />
        </div>
        <div class="inputfield">
          <h4>등급</h4>
            ...
            <div class="inputfield__input__userfield__autocomplete" v-if="showTargets">
              ...
              >
                <img
                  :src="
                    member.imgPath
                      ? $store.state.mainPath + member.imgPath
...
@Options({})
export default class AwardBoxCreatePopup extends Vue {
  members: any[] = [];
  title: string = '';
  subtitle: string = '';
  level: string = '';
  ...
  userInputKeyPress(e: any) {
    if (this.targetCurrentIndex >= this.getNotJoinMembers.length) this.targetCurrentIndex = this.getNotJoinMembers.length;
    switch (e.keyCode) {
      case 13:
      ...
      case 38:
        ...
        break;
      case 40:
        if (this.targetCurrentIndex < this.getNotJoinMembers.length - 1) this.targetCurrentIndex++;
        break;
    }
  }
  create() {
  ...
  get getNotJoinMembers(): any[] {
    return this.members.filter(
      (member: any) => this.target.findIndex((x) => member._id == x._id) == -1 && member.name.indexOf(this.searchTarget) != -1,
    );
  }
}
</script>
...
.darkTheme .award__createpopup__content__clear {
...
.darkTheme .award__createpopup__content__clear:hover {
  box-shadow: none;
  background-color: white;
  color: #282828;
}
...
.darkTheme .award__createpopup__content {
  color: white;
  background-color: #282828;
}
...
.darkTheme .award__createpopup__content h3 {
  color: white;
}
...
.darkTheme .award__createpopup__content__actions .create {
